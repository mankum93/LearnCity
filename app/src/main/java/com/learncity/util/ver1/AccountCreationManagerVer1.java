...
public class AccountCreationManagerVer1 {
    ...
    * that. Changing this implementation would probably have implications*/
    private ConcurrentLinkedQueue<BaseAccountCreationTaskVer1> tasksQueue = new ConcurrentLinkedQueue<BaseAccountCreationTaskVer1>();

    private BaseAccountCreationTaskVer1 currentTaskInExecution;
    ...
    public void startAccountCreation(BaseAccountCreationTaskVer1... tasks){
        ...
        tasksProcessor = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                BaseAccountCreationTaskVer1 task;
    ...
    private void performAccountCreation(final BaseAccountCreationTaskVer1 task){

        if(task == null){
            throw new NullPointerException("Can't process a null task!");
        }
        //Update the AC creation states for the task
        task.accountCreationTaskState = BaseAccountCreationTaskVer1.TASK_EXECUTION_STARTED;
        ...
        workerThread.execute(new Runnable() {
            ...
            public void run() {
                ...
                if(taskStatusCode == ACCOUNT_CREATION_COMPLETED){
                    ...
                    task.accountCreationTaskState = BaseAccountCreationTaskVer1.TASK_EXECUTION_COMPLETED;

                    //As soon as the AC gets created, make sure to execute client's code
                    if(task.accountCreationTaskListener != null){
                        Log.d(TAG, "AccountCreationManager.performAccountCreation(): " + "\n" + "MESSAGE: Calling AccountCreationTaskListener.onTaskProcessingComplete()..." +
                                "\n" +"Thread ID: " + Thread.currentThread().getId());
                        task.accountCreationTaskListener.onAccountCreated();
                    }
                    Log.d(TAG, "AccountCreationManager.performAccountCreation(): " + "\n" + "MESSAGE: Cleaning up the completed task; Calling BaseAccountCreationTaskVer1.onPerformCleanup()" +
                            "\n" +"Thread ID: " + Thread.currentThread().getId());
                    ...
                    Log.e(TAG, "AccountCreationManager.performAccountCreation(): " + "\n" + "MESSAGE: Task execution failed!!!" +
                            "\n" +"Thread ID: " + Thread.currentThread().getId());
                    //Update the AC creation state
                    task.accountCreationTaskState = BaseAccountCreationTaskVer1.TASK_EXECUTION_FAILED;
                    Thread t;
    ...
    public void retryOnFailedAccountCreation(){
        ...
        Log.d(TAG, "AccountCreationManager.retryOnFailedTask(): " + "\n" + "MESSAGE: retryAccountCreation() called..." +
                "\n" +"Thread ID: " + Thread.currentThread().getId());

        //This method would only be valid call if AC creation has failed
        if(currentTaskInExecution.accountCreationTaskState != BaseAccountCreationTaskVer1.TASK_EXECUTION_FAILED){
    ...
    public void cancelOnFailedAccountCreation(){
        ...
        Log.d(TAG, "AccountCreationManager.cancelOnFailedTask(): " + "\n" + "MESSAGE: retryAccountCreation() called..." +
                "\n" +"Thread ID: " + Thread.currentThread().getId());
